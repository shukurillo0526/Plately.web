  ...
  return (
    <div className="space-y-24 pb-20">
      ...
      <section className="text-center pt-12 max-w-4xl mx-auto px-4">
        <span className="text-xs font-bold text-brand-500 bg-brand-500/10 border border-brand-500/30 px-3 py-1 rounded-full uppercase tracking-wider">
          Inside Plately
        </span>
        ...
        <div className="flex flex-wrap justify-center gap-2 mt-8 max-w-3xl mx-auto bg-white/5 p-2 rounded-2xl border border-white/10">
          <button 
            onClick={() => setActiveTab('cook')}
            className={`px-4 py-2.5 rounded-xl text-xs font-bold transition-all ${activeTab === 'cook' ? 'glass-panel text-white shadow-sm' : 'text-slate-400 hover:text-slate-200'}`}
          >
            🍳 Cook Mode
          </button>
          <button 
            onClick={() => setActiveTab('scan')}
            className={`px-4 py-2.5 rounded-xl text-xs font-bold transition-all ${activeTab === 'scan' ? 'glass-panel text-white shadow-sm' : 'text-slate-400 hover:text-slate-200'}`}
          >
            📸 Scan & Digitize
          </button>
          <button 
            onClick={() => setActiveTab('shelf')}
            className={`px-4 py-2.5 rounded-xl text-xs font-bold transition-all ${activeTab === 'shelf' ? 'glass-panel text-white shadow-sm' : 'text-slate-400 hover:text-slate-200'}`}
          >
            🧺 Living Shelf
          </button>
          <button 
            onClick={() => setActiveTab('health')}
            className={`px-4 py-2.5 rounded-xl text-xs font-bold transition-all ${activeTab === 'health' ? 'glass-panel text-white shadow-sm' : 'text-slate-400 hover:text-slate-200'}`}
          >
            📊 Health & Intake
          </button>
          <button 
            onClick={() => setActiveTab('prep')}
            className={`px-4 py-2.5 rounded-xl text-xs font-bold transition-all ${activeTab === 'prep' ? 'glass-panel text-white shadow-sm' : 'text-slate-400 hover:text-slate-200'}`}
          >
            🍱 Bulk Meal Prep
          </button>
        </div>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-32">
        ...
        <div id="cook-mode-feature" className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
          <div className="lg:col-span-5 space-y-6">
            <div className="inline-flex items-center space-x-1 bg-brand-500/10 border border-brand-500/30 rounded-full px-3 py-1 text-xs text-brand-400 font-semibold">
            ...
            <div className="space-y-3">
              <div className="flex items-start space-x-2">
                <div className="p-1 rounded bg-brand-500/20 text-brand-400 mt-0.5">
            ...
            <div className="bg-brand-500/10 border border-brand-500/30 rounded-xl p-4 text-xs text-brand-400 font-medium">
          ...
          <div className="lg:col-span-7 glass-panel rounded-2xl border border-white/10 p-6 shadow-xl space-y-6">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between border-b border-white/5 pb-4 gap-2">
              <div>
                <h4 className="text-sm font-bold text-white">Plately Interactive Sandbox</h4>
                <p className="text-[11px] text-slate-500 font-sans">Simulate Plately's AI customizer and guided active cooking wizard</p>
              </div>
              <div className="flex bg-white/5 p-1 rounded-xl border border-white/10 shrink-0">
                <button 
                  onClick={() => setCookSubTab('simulator')}
                  className={`px-3 py-1.5 rounded-lg text-[10px] font-extrabold uppercase transition-all ${cookSubTab === 'simulator' ? 'bg-slate-900 text-white shadow-sm' : 'text-slate-400 hover:text-slate-200'}`}
                >
                  📱 Cook Mode
                </button>
                <button 
                  onClick={() => setCookSubTab('compiler')}
                  className={`px-3 py-1.5 rounded-lg text-[10px] font-extrabold uppercase transition-all ${cookSubTab === 'compiler' ? 'bg-slate-900 text-white shadow-sm' : 'text-slate-400 hover:text-slate-200'}`}
                >
                  🪄 AI Compiler
                </button>
              </div>
            ...
            {cookSubTab === 'compiler' ? (
              <div className="space-y-6 animate-fadeIn">
                <div className="space-y-3">
                  <p className="text-xs font-bold text-slate-300">1. SELECT INGREDIENTS ON YOUR SHELF:</p>
                    ...
                    {['Salmon', 'Spinach', 'Quinoa', 'Avocado', 'Onions', 'Almond Milk'].map(ing => {
                      ...
                      return (
                        <button
                          ...
                          className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-all flex items-center space-x-1 ${selected ? 'bg-brand-600 text-white border border-brand-700 shadow-sm' : 'bg-white/5 text-slate-400 border border-white/10 hover:bg-white/10/50'}`}
                        >
                          <span>{ing}</span>
                ...
                <button
                  ...
                  className="w-full py-3 bg-slate-900 hover:bg-slate-800 disabled:bg-white/10 disabled:text-slate-600 text-white text-xs font-bold rounded-xl transition-all shadow flex items-center justify-center space-x-1.5"
                >
                ...
                {generatedRecipe && (
                  <div className="bg-slate-950 rounded-xl p-4 border border-white/10 space-y-3 animate-fadeIn">
                    <div className="flex items-start justify-between">
                      <h5 className="text-xs font-extrabold text-white uppercase font-display">{generatedRecipe.title}</h5>
                      <span className="text-[10px] bg-brand-500/10 text-brand-400 font-bold px-2 py-0.5 rounded border border-brand-500/30 font-mono">
                        {generatedRecipe.prepTime}
                      </span>
                    </div>
                    <p className="text-[11px] text-slate-400 italic border-l-2 border-brand-500 pl-2">
                    ...
                    <div className="space-y-1.5">
                      {generatedRecipe.instructions.map((step, sIdx) => (
                        <div key={sIdx} className="flex items-start space-x-2 text-xs text-slate-300">
                          <span className="font-mono text-[10px] text-brand-500 bg-brand-500/10 w-4 h-4 rounded-full flex items-center justify-center font-bold flex-shrink-0 mt-0.5">
                            {sIdx + 1}
                          </span>
                          <span>{step}</span>
                        </div>
        ...
        <div id="scan-digitize-feature" className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
          <div className="lg:col-span-7 order-last lg:order-first">
            <div className="bg-slate-900 text-white rounded-2xl p-6 shadow-xl space-y-6 relative overflow-hidden">
              ...
              <div className="h-[220px] bg-slate-950 border border-slate-800 rounded-xl flex flex-col items-center justify-center p-4 text-center relative overflow-hidden">
                {scanStep === 0 && (
                  <div className="space-y-4">
                    ...
                    <div>
                      <p className="text-xs font-bold text-slate-200">Whole Foods Market grocery receipt ready</p>
                      <p className="text-[10px] text-slate-600">Press button below to simulate phone camera OCR scan</p>
                    </div>
                  </div>
                )}
                ...
                {scanStep === 2 && (
                  <div className="space-y-3">
                    ...
                    <p className="text-xs text-slate-400 font-mono">EXTRACTING INVENTORY COGNITIVE ENTITIES...</p>
                  </div>
                )}
                ...
                {scanStep === 3 && (
                  <div className="space-y-3 text-left w-full max-h-[180px] overflow-y-auto pr-2">
                    ...
                    <div className="bg-slate-900 border border-slate-800 rounded-lg p-3 space-y-1.5">
                      <div className="flex justify-between text-[11px] text-slate-600 border-b border-slate-800 pb-1">
                        <span>ITEM NAME</span>
              ...
              <div className="flex items-center justify-between gap-4">
                <button
                  ...
                  disabled={scanning}
                  className="flex-1 py-3 bg-brand-500 hover:bg-brand-600 disabled:bg-slate-800 disabled:text-slate-500 text-white hover:text-white font-bold text-xs rounded-xl transition-all shadow-md"
                >
          ...
          <div className="lg:col-span-5 space-y-6">
            ...
            <div className="space-y-4">
              <div className="flex items-start space-x-3">
                <div className="p-1 rounded-lg bg-white/5 text-slate-300 mt-0.5">
                  <ScanLine className="h-4 w-4" />
                </div>
                <div>
                  <h4 className="text-xs font-bold text-slate-200">Receipt OCR Scanning</h4>
              ...
              <div className="flex items-start space-x-3">
                ...
                <div>
                  <h4 className="text-xs font-bold text-slate-200">Photo Object Matching</h4>
                  <p className="text-[11px] text-slate-500 leading-relaxed">
        ...
        <div id="living-shelf-feature" className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
          ...
          <div className="lg:col-span-7 glass-panel rounded-2xl border border-white/10 p-6 shadow-xl space-y-4">
            <div className="flex items-center justify-between border-b border-white/5 pb-3">
              <h4 className="text-xs font-bold text-slate-300 uppercase tracking-wide">Interactive Freshness Map</h4>
            ...
            <div className="space-y-2.5">
              ...
              <div className="border border-red-200 bg-red-50/50 p-3.5 rounded-xl flex items-center justify-between shadow-sm">
                ...
                <div className="flex items-center space-x-2">
                  ...
                  <button className="px-2 py-1 glass-panel hover:bg-white/5 border border-white/10 text-slate-400 rounded-lg text-[10px] font-bold transition-all">
                    Freeze ❄️
                  </button>
                </div>
              </div>
              ...
              <div className="border border-white/10 glass-panel p-3.5 rounded-xl flex items-center justify-between shadow-sm">
                <div className="space-y-1">
                  <div className="flex items-center space-x-1.5">
                    <span className="text-xs font-bold text-white">Organic Avocados</span>
                    <span className="text-[9px] bg-white/5 text-slate-500 font-mono px-1.5 rounded">Countertop</span>
                  </div>
                  <div className="w-48 bg-white/10 h-1 rounded-full overflow-hidden">
        ...
        <div id="health-intake-feature" className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
          <div className="lg:col-span-7 order-last lg:order-first">
            <div className="glass-panel rounded-2xl border border-white/10 p-6 shadow-xl space-y-6">
              <div className="flex items-center justify-between border-b border-white/5 pb-3">
                <div>
                  <h4 className="text-sm font-bold text-slate-200">Macro Analytics Board</h4>
                  <p className="text-[11px] text-slate-500 font-sans">Simulating smart nutritional feedback loops</p>
                </div>
                <span className="text-[10px] bg-brand-500/10 text-brand-400 border border-brand-500/30 px-2.5 py-0.5 rounded-full font-bold">
                  Daily Intake
                </span>
              </div>
              ...
              <div className="grid grid-cols-3 gap-3">
                <div className="border border-white/10 p-3 rounded-xl bg-slate-950/50">
                  <span className="text-[10px] text-slate-500 uppercase font-mono tracking-wider">PROTEIN</span>
                  <p className="text-xl font-bold font-display text-white mt-1">112g</p>
                  <p className="text-[9px] text-slate-600 font-sans mt-0.5">83% of daily goal</p>
                </div>
                <div className="border border-white/10 p-3 rounded-xl bg-slate-950/50">
                  <span className="text-[10px] text-slate-500 uppercase font-mono tracking-wider">CARBOHYDRATES</span>
                  <p className="text-xl font-bold font-display text-white mt-1">194g</p>
                  <p className="text-[9px] text-brand-500 font-bold mt-0.5">Optimal levels</p>
                </div>
                <div className="border border-white/10 p-3 rounded-xl bg-slate-950/50">
                  <span className="text-[10px] text-slate-500 uppercase font-mono tracking-wider">HEALTH SCORE</span>
                  <p className="text-xl font-bold font-display text-emerald-600 mt-1">A-</p>
                  <p className="text-[9px] text-slate-600 font-sans mt-0.5">Based on whole food %</p>
                </div>
              </div>
              ...
              <div className="bg-brand-500/10 border border-brand-500/30 rounded-xl p-4 space-y-2">
                <div className="flex items-center space-x-1.5">
                  ...
                  <span className="text-xs font-bold text-brand-300">Plately AI Intake Estimate</span>
                </div>
                <p className="text-[11px] text-brand-400 leading-relaxed font-sans">
        ...
        <div id="meal-prep-feature" className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
          ...
          <div className="lg:col-span-7 bg-slate-900 text-white rounded-2xl p-6 shadow-xl space-y-4">
            <div className="flex items-center justify-between border-b border-slate-800 pb-3">
              <h4 className="text-xs font-mono font-bold tracking-wider">PREP BOX EXPIRY CALENDAR</h4>
              <span className="text-[10px] bg-brand-500 text-white font-bold px-2 py-0.5 rounded">Active Plan</span>
            </div>

            <div className="space-y-2 text-xs font-sans">
              <div className="bg-slate-800 p-3.5 rounded-xl border border-slate-700 flex items-center justify-between">
                <div>
                  <h5 className="font-bold text-slate-200">Box 1-3: Chili Lime Beef Bowls</h5>
                  <p className="text-[10px] text-slate-600">Prepped: Sunday (3 days ago) • Fridge Zone 3</p>
                </div>
              ...
              <div className="bg-slate-800 p-3.5 rounded-xl border border-slate-700 flex items-center justify-between">
                <div>
                  <h5 className="font-bold text-slate-200">Box 4-6: Honey Garlic Salmon Medleys</h5>
                  <p className="text-[10px] text-slate-600">Prepped: Sunday (3 days ago) • Freezer Chest</p>
                </div>
